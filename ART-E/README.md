...
**自前の GPU で LocalBackend を使う場合は `local` extras を追加:**
...
uv run python art_e.py

# カスタムパラメータ
uv run python art_e.py --project my-project --max-steps 100 --groups-per-step 4

# 自前の GPU を使う場合（LocalBackend）
uv run python art_e.py --local

# LocalBackend のデータ保存先を指定
uv run python art_e.py --local --local-path /data/art
```
...
uv run python eval_model.py --artifact-path "your-entity/your-project/model-name:version"

# ローカル GPU で評価（vLLM サーバーを自動起動）
uv run python eval_model.py --artifact-path "your-entity/your-project/model-name:version" --local
```
